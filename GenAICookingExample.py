...
data = {
    'age': [29],
    'gender': ['male'],
    'conversation_history': ["I am trying to cook italian meal."],
    'cooking_state': ["italian"],
    'preferred_support_style': ["insightful and data-driven"],
    'topics_of_interest': ["pizza, sauce, olive oil"],
...
def generate_ai_response(response_type, user_input):
    system_message = "You are a cooking assistant."
    ...
    if response_type in ["With Data Science Only", "With Both Data Science and UX"]:
        ...
        customization_info = f"User is a {user_info['age']}-year-old {user_info['gender']} who is currently feeling {user_info['cooking_state']}. They previously mentioned: '{user_info['conversation_history']}' and prefer '{user_info['preferred_support_style']}'. They are interested in topics like '{user_info['topics_of_interest']}' and have given feedback that they '{user_info['previous_advice_feedback']}'."
    else:
    ...
    if response_type == "Without Data Science and UX":
        ...
        explanation = "This response is a general recommendation based on italian cusine principles."
        ...
        explanation = "This response leverages the user's cooking interests and conversation history, along with relevant resources."
    elif response_type == "With UX Only":
        prompt = f"The user seeks support and here is their input: '{user_input}'. Provide an culinary response with visual and sensory elements."
        explanation = "This response focuses on culinary language and aims to provide comfort with visual and audio aids."
        ...
        explanation = "This response combines data-driven insights with culinary language, considering user history and online resources. The response should have metrics and numbers and percentages for each suggestion and also links the user can review."
        explanation = "This response leverages the user's culinary interests and conversation history, along with relevant resources."
...
# Streamlit UI
st.set_page_config(page_title="AI Cooking Coach", page_icon=":brain:", layout="wide")
...
st.markdown('<div class="title">AI Cooking Coach</div>', unsafe_allow_html=True)
st.markdown('<div class="intro-text">Share your thoughts or experiences to receive personalized cooking advice.</div>', unsafe_allow_html=True)
